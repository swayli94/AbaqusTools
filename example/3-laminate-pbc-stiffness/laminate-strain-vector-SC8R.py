'''
Test the Periodic Boundary Condition (PBC) in Abaqus with a simple laminate beam.
...
class Beam(Part):
    ...
    def __init__(self, model, name_part, pMesh, length_x=100, length_y=20, length_z=150, 
                    seedPart_size=5):
        
        super(Beam,self).__init__(model, None, pMesh)
    ...
    def set_section_assignment(self):
        ...
        self.set_CompositeLayup_of_set(myPrt, 
                name_set=       'all', 
                total_thickness=self.length_y, 
                ply_angle=      self.pMesh['CompositePly_orientationValue'],
                eNum_thickness= self.pMesh['num_element_thickness'],
                symmetric=      self.pMesh['CompositeLayup_symmetric'], 
                numIntPoints=   3,
                normalDirection=AXIS_2,
                )


class TestModel(Model):
    
    def __init__(self, name_job, pMesh, strain_vector=[1E-6,0,0,0]):
        
        super(TestModel,self).__init__(pGeo=None, pMesh=pMesh, pRun=None)
    ...
    def setup_parts(self):
        ...
        self.beam_0 = Beam(self.model, 'beam_0', self.pMesh, length_x=100, length_y=20, length_z=150, seedPart_size=4.0)
        self.beam_0.build()
    ...
    def create_periodic_bc(self):
        ...
        self.create_reference_point(0, 10, self.beam_0.length_z*1.1, 'MasterNode-2')
...
if __name__ == '__main__':
    ...
    pMesh = {}
    pMesh['failure_model'] = 'Default'
    pMesh['composite_ply_thickness'] = 0.5
    pMesh['CompositePly_orientationValue'] = [45, -45, 0, 90, 0]
    pMesh['num_element_thickness'] = 4
    pMesh['CompositeLayup_symmetric'] = True
    ...
    name_job = 'Job_laminate_%d'%(i0)

    model = TestModel(name_job, pMesh, STRAIN_VECTORS[i0,:])
    model.build()
    
    model.write_job_inp(model.name_job)
    model.save_cae('laminate_%d.cae'%(i0))
