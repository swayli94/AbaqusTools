'''
Slope-ratio analysis comparing FEM SC8R and IM S4R against FEM C3D8R (reference).
...
Output files (./error-analysis/):
    ...
    {label}_bar_{metric}.png     — grouped bar charts for slope metrics
'''
...
from compare_ply_stress_field import (
    N_CASE, NAME_COMPONENTS_PLOT,
    SOURCES, SOURCE_LABELS, CASE_LABELS, N_SRC, N_COMP,
    R_OUTER_RATIO,
    create_unit_template_mesh,
    derive_ply_info, collect_fields,
)

RF_COMPONENTS = ['RF1', 'RF2', 'RF3']
N_RF = len(RF_COMPONENTS)

def _job_prefix(source: str) -> str:
    return 'Job_BJ' if source in ('fem_C3D8R', 'fem_SC8R') else 'Job_MIF'


def load_rf_loads(i_sample: int, i_case: int, source: str, path_data: str) -> np.ndarray:
    '''
    Parse {job}-RF.dat and return [RF1, RF2, RF3] as a float array.
    Returns array of nan if the file is missing or the keys are absent.
    '''
    prefix = _job_prefix(source)
    fname  = os.path.join(path_data, f'{prefix}_{i_sample}_{i_case}-RF.dat')
    values = {f'RF{i}': np.nan for i in range(1, 4)}
    if not os.path.exists(fname):
        print(f'  Warning: {fname} not found')
        return np.array([np.nan] * N_RF)
    with open(fname) as f:
        for line in f:
            parts = line.split()
            if len(parts) == 2 and parts[0].startswith('RF_load_RF'):
                key = parts[0][len('RF_load_'):]   # e.g. 'RF1'
                if key in values:
                    try:
                        values[key] = float(parts[1])
                    except ValueError:
                        pass
    return np.array([values[f'RF{i}'] for i in range(1, 4)])


def collect_rf_loads(i_sample_dict: dict,
                     func_path_data) -> np.ndarray:
    '''
    Load RF1/RF2/RF3 for all (case x source) combinations.

    Returns
    -------
    np.ndarray, shape (N_CASE, N_SRC, N_RF)
    '''
    out = np.full((N_CASE, N_SRC, N_RF), np.nan)
    for i_case in range(N_CASE):
        for i_src, src in enumerate(SOURCES):
            out[i_case, i_src] = load_rf_loads(
...
def plot_rf_comparison(rf_loads: np.ndarray, label: str):
    '''
    Grouped bar chart: RF_load RF1 / RF2 / RF3 across sources and cases.

    Layout: 1 row x N_RF columns.
    Each subplot shows one RF component; grouped bars = sources; x-ticks = cases.
    '''
    x     = np.arange(N_CASE)
    width = 0.8 / N_SRC

    fig, axes = plt.subplots(1, N_RF,
                             figsize=(N_RF * 4.0, 3.5),
                             squeeze=False)

    for i_rf, rf_name in enumerate(RF_COMPONENTS):
        ax = axes[0][i_rf]
        for i_src, src_label in enumerate(SOURCE_LABELS):
            vals   = rf_loads[:, i_src, i_rf]          # shape (N_CASE,)
            offset = (i_src - (N_SRC - 1) / 2.0) * width
            ax.bar(x + offset, vals, width=width * 0.92,
                   label=src_label, zorder=2)

        ax.axhline(0.0, color='dimgray', ls='--', lw=0.8)
        ax.set_xticks(x)
        ax.set_xticklabels(CASE_LABELS, fontsize=8)
        ax.set_ylabel(f'RF_load_{rf_name} (N)', fontsize=8)
        ax.set_title(f'RF_load_{rf_name}', fontsize=9)
