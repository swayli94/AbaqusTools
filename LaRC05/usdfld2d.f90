...
! this subroutine forces the material behaviour to 2D only
...
!  -------      --------------------------------------------------------!
    ...
    ! cycles
    integer             ::  i, NSTR=-1
    integer             ::  nFieldTenComp = -1, nFIs = 4, &
    ...
    if (NSTATV .GT. SDV_NUMBER_2D) then
        nFIMax = 9  ! if we have space for FImax
    end if
    if (NFIELD .GT. FV_NUMBER_2D) then
        NFIELDDMG = 3       ! 3 damage variables (relevant for range)
        nFieldTenComp = 4   ! space for the TC variable
    end if
